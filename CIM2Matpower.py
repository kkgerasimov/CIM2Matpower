...
def cim_to_mpc(cimfiles, boundary_profiles=[], log_filename=''):
    ...
    .. warning:: The *cimfiles* usually come in three files: _EQ, _TP and _SV. If so, they need to be provided in the specified order!
                ...
                if len(pt.TransformerWindings) == 2:
                    ...
                    if ptc:
                        ...
                        if ptc.phaseTapChangerType == 'symmetrical':
                            ...
                            raise Exception('Unknown phaseTapChangerType!'), ptc.phaseTapChangerType
                        ...
                        for angle_deg in angle_shift_deg_list:
                            ...
                            if xStepRangeIsInconsistent or angle_max_rad == 0:
                                ...
                                if ptc.phaseTapChangerType == 'symmetrical':
                                    x_ohms = ptc.xStepMin + (ptc.xStepMax - ptc.xStepMin) * math.pow(
                                        math.sin(angle_rad / 2) / math.sin(angle_max_rad / 2), 2)
                                elif ptc.phaseTapChangerType == 'asymmetrical':
                                    ...
                                    x_ohms = ptc.xStepMin + (ptc.xStepMax - ptc.xStepMin) * math.pow(
                                        math.tan(angle_rad) / math.tan(angle_max_rad) * numer / denom, 2)
                    ...
                    if pt.TransformerWindings[index_primary].RatioTapChanger:
                        if pt.TransformerWindings[index_secondary].RatioTapChanger:
                            ...
                            rtc = pt.TransformerWindings[index_secondary].RatioTapChanger
                            if rtc:
                                rtc_ratio = 1 / (1 + (rtc.SvTapStep.continuousPosition - rtc.neutralStep) * (
                                    rtc.stepVoltageIncrement / 100))

                    # Limits
                    rate_a_mva = pt.TransformerWindings[
                        index_primary].ratedS
